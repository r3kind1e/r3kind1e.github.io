...
标志 1 存在于可公开访问的共享中名为flag_1的文件中。
标志 2 存在于其中一个用户的不可浏览共享中名为flag_2的目录中。
标志 3 存在于另一个不可浏览共享中名为flag_3的文件中。
...
信息：当 Nmap 无法确定端口是打开还是过滤时，Nmap 会将端口置于“打开|过滤”状态。这发生在开放端口没有响应的扫描类型中。没有响应也可能意味着数据包过滤器丢弃了探测。
